...
![em框确定内容区高度](../images/emdeterminsline-height.JPG)

行内框的生成： line-height（21px）减去font-size（15px）之后均分两半（3px）分别应用到内容区的顶部和底部，这就得到了行内框。

![内容区加上行间距就等于行内框](../images/inlineelement.JPG)
...
![行中的行内框](../images/inline-box-in-a-row.JPG)
...
![段落中的行框](../images/inline-box-in-a-paragraph.JPG)
...
![垂直对齐影响行框高度](../images/vertical-align-affects-inline-box-height.JPG)
